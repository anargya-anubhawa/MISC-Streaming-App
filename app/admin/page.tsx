...
export default function AdminPage() {
  ...
  const toggleUnlockAccess = async (uid: string, unlocked?: boolean) => {
    if (!uid) return;
  ...
  const resetAllAccess = async () => {
    const confirmed = confirm(
      "Matikan akses video untuk semua user? Admin perlu menyalakan ulang akses secara manual untuk user yang membutuhkan."
    );
    ...
    try {
      ...
      alert("Akses video semua user berhasil dimatikan.");
    } catch {
      alert("Gagal mematikan akses semua user.");
    } finally {
    ...
    <div className="space-y-10 pb-12">
      ...
      <section className="dashboard-shell">
        <div className="dashboard-shell-header">
          <div>
            ...
            <h1 className="text-3xl font-semibold text-[var(--foreground-strong)] mt-2">
      ...
      <section className="card p-6">
        <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            <h2 className="text-2xl font-bold text-[var(--foreground-strong)]">User Management</h2>
          ...
          <div className="flex w-full flex-col gap-3 md:w-auto md:min-w-[26rem]">
            ...
            >
              {resettingAccess ? "Mematikan Akses..." : "Matikan Semua Akses"}
            </button>
          </div>
        </div>
          ...
          <table className="w-full border border-[var(--border)] text-sm">
            <thead>
              <tr className="bg-[var(--surface)]">
                <th className="p-3 border border-[var(--border)]">Nama</th>
                <th className="p-3 border border-[var(--border)]">NIM</th>
                <th className="p-3 border border-[var(--border)]">Email</th>
                <th className="p-3 border border-[var(--border)]">Phone</th>
                <th className="p-3 border border-[var(--border)]">Status</th>
                <th className="p-3 border border-[var(--border)]">Akses Video</th>
                <th className="p-3 border border-[var(--border)]">Action</th>
              </tr>
            </thead>
              ...
              {filteredUsers.map((user) => {
                ...
                return (
                  <Fragment key={user.id}>
                    ...
                    >
                      <td className="p-3 border border-[var(--border)]">{user.name || "-"}</td>
                      <td className="p-3 border border-[var(--border)]">{user.nim || "-"}</td>
                      <td className="p-3 border border-[var(--border)]">{user.email || "-"}</td>
                      <td className="p-3 border border-[var(--border)]">{user.phone || "-"}</td>
                      ...
                      <td className="p-3 border border-[var(--border)] text-center">
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={Boolean(user.isUnlocked)}
                            onChange={() =>
                              void toggleUnlockAccess(user.id, user.isUnlocked)
                            }
                            disabled={user.role === "admin"}
                            className="h-5 w-5 accent-green-400"
                          />
                          <span className={user.isUnlocked ? "text-[var(--accent)]" : "text-[var(--muted)]"}>
                            {user.isUnlocked ? "On" : "Off"}
                          </span>
                        </label>
                      </td>
                      <td className="p-3 border border-[var(--border)] text-center">
                    ...
                    {isOpen && (
                      <tr>
                        <td
                          colSpan={7}
                          className="p-4 bg-[var(--background-elevated)] border border-[var(--border)] text-xs"
                        >
                          <div className="grid md:grid-cols-2 gap-2">
                            <p><b>UID:</b> {user.id}</p>
                            <p><b>Device:</b> {user.deviceInfo || "-"}</p>
                            <p><b>Session:</b> {(user.activeSessionIds || [user.activeSessionId]).filter(Boolean).join(", ") || "-"}</p>
                            <p><b>Last Login:</b> {formatDateTime(user.lastLogin)}</p>
                            <p>
                              <b>Akses Video:</b>{" "}
                              <span className={user.isUnlocked ? "text-[var(--accent)]" : "text-[var(--danger)]"}>
                                {user.isUnlocked ? "Aktif" : "Nonaktif"}
                              </span>
                            </p>
                          </div>
                        </td>
      ...
      <section className="card p-6 space-y-5">
        <div className="flex flex-col gap-2">
          <h2 className="text-2xl font-bold text-[var(--foreground-strong)]">Blok</h2>
        ...
        {blockOptions.map((block) => (
          <div key={block.id} className="admin-list-card">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <p className="text-[var(--foreground-strong)] font-semibold">{block.name}</p>
                  ...
                  .map((video) => (
                    <div key={video.id} className="admin-subitem-card">
                      <div className="space-y-1">
                        <p className="text-[var(--foreground-strong)] font-medium">{video.title}</p>
                      </div>
      ...
      <section className="card p-6 space-y-5">
        <div>
          <h2 className="text-2xl font-bold text-[var(--foreground-strong)]">Tambah Video</h2>
          <p className="text-[var(--muted)] mt-2">
      ...
      <section className="card p-6 space-y-5">
        <div>
          <h2 className="text-2xl font-bold text-[var(--foreground-strong)]">Pengaturan Berlangganan</h2>
          <p className="text-[var(--muted)] mt-2">
        ...
        <div className="admin-form-group">
          ...
          <div className="admin-form-fields">
            <label className="flex items-center justify-between gap-4 rounded-2xl border border-green-400/15 bg-[var(--surface-muted)] px-4 py-3">
              <span>
      ...
      {editingBlock && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <div className="card p-6 w-full max-w-xl space-y-4">
            <h2 className="text-lg font-bold text-[var(--foreground-strong)]">Edit Blok</h2>
      ...
      {editingVideo && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <div className="card p-6 w-full max-w-md space-y-4">
            <h2 className="text-lg font-bold text-[var(--foreground-strong)]">Edit Video</h2>
